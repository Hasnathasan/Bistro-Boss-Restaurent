  ...
  return (
    <div>
      <Cover
        img={orderImg}
        header="Order Food"
        subTitle="Lorem ipsum dolor sit, amet consectetur adipisicing elit. Incidunt quos delectus porro doloribus ut. Tempora reprehenderit"
      ></Cover>
      <div className="my-32">
        <Tabs selectedIndex={tabIndex} onSelect={(index) => setTabIndex(index)}>
          ...
          <TabPanel>
            <div className="grid grid-cols-1 md:grid-cols-3 px-4 md:px-20 gap-10">
              {pizzas.map((pizza) => (
                <FoodCard key={pizza._id} food={pizza}></FoodCard>
              ))}
            </div>
          </TabPanel>
          <TabPanel>
            <div className="grid grid-cols-1 md:grid-cols-3 px-4 md:px-20 gap-10">
              {soups.map((soup) => (
                <FoodCard key={soup._id} food={soup}></FoodCard>
              ))}
            </div>
          </TabPanel>
          <TabPanel>
            <div className="grid grid-cols-1 md:grid-cols-3 px-4 md:px-20 gap-10">
              {desserts.map((dessert) => (
                <FoodCard key={dessert._id} food={dessert}></FoodCard>
              ))}
            </div>
          </TabPanel>
          <TabPanel>
            <div className="grid grid-cols-1 md:grid-cols-3 px-4 md:px-20 gap-10">
              {drinks.map((drink) => (
                <FoodCard key={drink._id} food={drink}></FoodCard>
              ))}
            </div>
          </TabPanel>
        </Tabs>
